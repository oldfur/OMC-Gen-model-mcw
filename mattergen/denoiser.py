...
class GemNetTDenoiser(ScoreModel):
    ...
    def __init__(
        ...
        molecule_conditioner: torch.nn.Module | None = None,
        molecule_conditioner_gate_center: float | None = None,
        molecule_conditioner_gate_width: float = 0.05,
        molecule_conditioner_gate_min_scale: float = 0.0,
        ...
        **kwargs,
        ...
        self.molecule_conditioner = molecule_conditioner
        self.molecule_conditioner_gate_center = molecule_conditioner_gate_center
        self.molecule_conditioner_gate_width = molecule_conditioner_gate_width
        self.molecule_conditioner_gate_min_scale = molecule_conditioner_gate_min_scale
    ...
    def forward(
        ...
        node_condition = (
            self.molecule_conditioner(x) if self.molecule_conditioner is not None else None
        )
        if node_condition is not None and self.molecule_conditioner_gate_center is not None:
            gate = torch.sigmoid(
                (float(self.molecule_conditioner_gate_center) - t)
                / float(self.molecule_conditioner_gate_width)
            )
            min_scale = float(self.molecule_conditioner_gate_min_scale)
