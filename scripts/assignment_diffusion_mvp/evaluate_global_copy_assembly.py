...
def main():
    ...
    cfg,sample,target,tree,model=load_setup(args.config);model.load_state_dict(torch.load(args.checkpoint,map_location="cpu",weights_only=False)["state_dict"]);model.eval()
    ...
    (output/"global_copy_assembly_report.md").write_text("# Global copy assembly evaluation\n\nStatus: `CLEAN_GEOMETRY_ORACLE_R_ONLY`. This report does not establish noisy-geometry or pos/cell denoising benefit.\n")
