#!/usr/bin/env bash
# Paired Original vs Clean-G geometry ablation (RHODIN01, 1000 steps, seed 17).
# Denoising only: SCF sees A_0^GT at every t; no G/R learning; no sampling.
set -euo pipefail
...
BASE="outputs/assignment_diffusion_mvp/clean_g_geometry_ablation"
...
echo "[0] smoke Original vs Clean-G"
python scripts/assignment_diffusion_mvp/smoke_clean_g_geometry_ablation.py --config "$CFG" --execute "${MG[@]}"
if [[ "$SMOKE" == "true" ]]; then
...
python scripts/assignment_diffusion_mvp/compare_clean_g_geometry_ablation.py \
  --original "$BASE/original" --clean-g "$BASE/clean_g" \
  --out "$BASE/comparison.json"

echo "DONE Clean-G → geometry ablation → $BASE"
